...
export const caseFiles: CaseFile[] = [
  {
    ...
    system: "Authenticated gateway enforcing role-based access and reason-for-access policy, with request and token budgets plus an optional Redis-backed atomic limiter path.",
    evidence: "Audit logs, trace export, Grafana dashboards, and incident runbooks. Release gates cover shadow, canary, staged rollout, and rollback. Supply-chain checks in CI scan the container image and pin every dependency.",
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving with authenticated benchmark agents, plus a controlled local lifecycle qualifier that measures readiness before benchmark work. Its deterministic CI fixtures cover verified HTTPS with an explicit CA bundle, bounded restart reconciliation, and no duplicate target work after one coordinator or agent response is lost.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with authenticated coordinator-agent execution, an opt-in TLS 1.2+ agent listener, explicit CA verification, HMAC-bound coordinator state, exact-request status reconciliation, bounded in-memory and SQLite agent result recovery, NTP-style clock sampling, conservative time bounds, open-loop pacing, privacy-safe aggregate gates, and a shell-free local service-lifecycle wrapper that measures process launch to an explicit health response before invoking the benchmark.",
    evidence: "The TLS fixture generates an ephemeral certificate, runs two authenticated HTTPS agents with an explicit CA bundle, rejects an invalid bearer key and an untrusted CA before target work, and checks redacted JSON/Prometheus artifacts. The published benchmark_report.py compares ordered saved JSON runs with explicit p95-latency, throughput, success-rate, and retry-amplification gates while omitting endpoints and prompts. The lifecycle qualification fixture starts a delayed local service, measures process-launch to HTTP-200 readiness, runs an OpenAI-compatible request path, terminates the service, and omits the command, health URL, source path, prompt, and authorization data from its JSON/Markdown projection. Separate fixtures recover one completed shard across an agent restart and both shards after coordinator restart; the synthetic target remains at exactly eight requests with zero duplicates. This is single-host synthetic protocol evidence; the lifecycle value is not model cold-start time, partial-workflow continuation, interrupted-child recovery, multi-host behavior, or a production-network measurement.",
...
export const proof: Proof[] = [
  ...
  { label: "restart reconciliation", kind: "verified",
    plain: "Verified HTTPS CI runs two authenticated agents with an explicit CA bundle, then recovers a completed shard across an agent restart and both shards across a coordinator restart without duplicate target work; partial or multi-host recovery is not claimed.",
    href: "https://github.com/WaffleBits/triton-inference-benchmark#coordinate-authenticated-agents" },
  { label: "controlled readiness", kind: "verified",
    plain: "A real delayed-readiness fixture starts a local service without a shell, measures process launch to HTTP-200 readiness, runs the benchmark, and keeps command and endpoint values out of the report.",
    href: "https://github.com/WaffleBits/triton-inference-benchmark/blob/main/lifecycle_qualification.py" },
  { label: "spending limits", kind: "operational",
    plain: "Optional Redis-backed limits that stay correct across many servers at once, without storing who made the request.",
  ...
  { label: "backend probe", kind: "measured",
    plain: "A short live check against the model endpoint, recording how often it succeeded and how fast.",
    href: "https://github.com/WaffleBits/secure-gpu-inference-gateway/blob/main/artifacts/backend-probe-evidence.json" },
  { label: "telemetry correlation", kind: "operational",
    plain: "One snapshot tying request outcomes, token usage, and latency together.",
    href: "https://github.com/WaffleBits/secure-gpu-inference-gateway/blob/main/artifacts/telemetry-correlation-evidence.json" },
  { label: "deployment safety", kind: "verified",
    plain: "Automated checks that scan the container, list every dependency, and test the locked-down runtime.",
