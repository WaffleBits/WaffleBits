...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving with authenticated benchmark agents. Its deterministic CI fixture terminates a coordinator after two agents complete, then a second coordinator verifies and retrieves both results without repeating any target request.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with authenticated coordinator-agent execution, opt-in HMAC-bound coordinator state, exact-request status reconciliation, bounded in-memory and SQLite agent result recovery, NTP-style clock sampling, conservative time bounds, open-loop pacing, and privacy-safe aggregate gates.",
    evidence: "A deterministic CI fixture launches two agent services, withholds both completed responses, terminates the first coordinator, and starts a second process from the original owner-only authenticated manifest. The second process verifies and retrieves both completed shards while the synthetic target remains at exactly eight unique requests. The fixture also rejects a wrong token and checks that credentials, raw identities, arguments, prompts, URLs, paths, result bodies, and trace IDs are absent from persisted state and aggregate artifacts. A separate fixture recovers one completed shard across an agent restart with SQLite. This is single-host synthetic protocol evidence, not partial-workflow continuation, interrupted-child recovery, multi-host behavior, or a production-network measurement.",
...
export const proof: Proof[] = [
  ...
  { label: "restart reconciliation", kind: "verified",
    plain: "Synthetic CI recovers a completed shard across an agent restart and all eight requests across a coordinator restart without duplicate target work; partial or multi-host recovery is not claimed.",
