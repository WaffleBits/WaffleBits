...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving: how fast is it, where are requests dropped, and what does each request cost. In a committed synthetic test, one server process exits and a separate local supervisor starts its replacement.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with open-loop pacing, a fixed recorded retry delay, concurrent multi-source Prometheus capture, and privacy-safe request-path and lifecycle gates.",
    evidence: "CI triggers a controlled backend-process exit on its second receipt; a separate supervisor starts a replacement on the same local port. Four logical requests produce six client and ingress attempts, five backend receipts, four completions, two retry recoveries, and one completed restart; path, retry, trace, and lifecycle gates pass. Artifacts omit endpoint URLs, raw scrapes, raw metric names, labels, prompts, outputs, and trace IDs. Aggregate agreement is not per-request causality or service MTTR.",
...
export const proof: Proof[] = [
  ...
  { label: "supervised service restart", kind: "verified",
    plain: "A synthetic CI run makes a local backend process exit, starts a replacement under a separate supervisor, and reconciles the completed restart with independent path and retry counters.",
    href: "https://github.com/WaffleBits/triton-inference-benchmark#features" },
