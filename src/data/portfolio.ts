...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving: how fast is it, where are requests dropped, and what does each request cost. A committed synthetic test sends streamed requests through separate local router and server processes.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with open-loop pacing, measured retries, and concurrent capture from independent Prometheus endpoints.",
    evidence: "CI injects one router-local and one backend 503 on different synthetic requests. Four logical requests produce six client attempts, six ingress receipts, five backend receipts, four completions, and two retry recoveries; exact path, retry, and trace gates pass. Artifacts omit endpoint URLs, raw scrapes, metric names, labels, prompts, outputs, and trace IDs. Aggregate agreement is not per-request causality.",
...
export const proof: Proof[] = [
  ...
  { label: "multi-process path gate", kind: "verified",
    plain: "A synthetic CI run sends streamed requests through separate local router and backend processes, injects failures at both boundaries, and reconciles independent counters with retries.",
