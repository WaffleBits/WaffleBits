...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving with authenticated benchmark agents. Its deterministic restart fixture drops one successful result response, restarts the agent, then recovers the completed result without running that benchmark child twice.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with authenticated coordinator-agent execution, bounded in-memory and opt-in SQLite result recovery, NTP-style clock sampling, conservative time bounds, open-loop pacing, and privacy-safe aggregate gates.",
    evidence: "A deterministic CI fixture launches two successive agent processes around one real benchmark child. After an intermediary discards the successful response, the second process returns the completed projection from the same SQLite state with durable provenance. The synthetic target still records exactly two unique requests, proving no duplicate target work. The fixture also checks conflict rejection, database integrity, owner-only permissions, and that credentials, raw identities, prompts, URLs, paths, and trace IDs are absent from persisted state. Separate CI covers the existing two-agent, eight-request coordination path. This is local synthetic protocol evidence, not coordinator restart recovery, interrupted-child recovery, multi-host storage, or a production-network measurement.",
...
export const proof: Proof[] = [
  ...
  { label: "agent result recovery", kind: "verified",
    plain: "A synthetic CI fixture drops one successful response, restarts the agent, and recovers the completed result from opt-in SQLite state without duplicate target work; multi-host behavior is not claimed.",
