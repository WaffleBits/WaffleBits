...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving that can coordinate independent client processes on one host. A deterministic local fixture completes eight requests across two clients while checking configuration, start skew, overlapping windows, and trace uniqueness.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with open-loop pacing, same-host process coordination, multi-source Prometheus capture, and privacy-safe request-path and lifecycle gates.",
    evidence: "CI launches two independent benchmark clients against one deterministic local SSE server. Eight logical requests complete with eight unique trace contexts; complete-client, common-configuration, start-skew, overlap, and trace gates pass. Aggregate artifacts omit child paths, endpoints, prompts, outputs, and trace IDs, and mark global latency percentiles unavailable because child quantiles cannot be merged. This is not multi-node load or cross-host clock evidence.",
...
export const proof: Proof[] = [
  ...
  { label: "coordinated client processes", kind: "verified",
    plain: "A synthetic CI run launches two local benchmark clients against one planned start and validates all eight requests, eight unique traces, overlapping windows, and bounded start skew.",
