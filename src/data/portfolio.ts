...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving with authenticated benchmark agents. Its deterministic CI fixture drops one successful result response, then recovers the completed result without running that benchmark child twice.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with authenticated coordinator-agent execution, bounded in-memory idempotent result recovery, NTP-style clock sampling, conservative time bounds, open-loop pacing, and privacy-safe aggregate gates.",
    evidence: "CI starts two authenticated loopback agents, two real benchmark child processes, and one synthetic SSE target. It deliberately discards one successful result response, then retrieves a coordinator-only cached projection on one transport retry. Eight target requests complete with eight unique trace contexts, showing that the affected child was not executed twice. Wrong-key and conflicting replay attempts are rejected; conservative clock-uncertainty, start-skew, and overlap gates pass. Aggregate artifacts omit keys, URLs, child paths, endpoints, prompts, outputs, and trace IDs. This is single-host synthetic protocol evidence, not persistent recovery, multi-host behavior, or a production-network measurement.",
...
export const proof: Proof[] = [
  ...
  { label: "agent result recovery", kind: "verified",
    plain: "A synthetic CI fixture drops one successful agent response and recovers the completed result without duplicate benchmark execution; persistence and multi-host behavior are not claimed.",
