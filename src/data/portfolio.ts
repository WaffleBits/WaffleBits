...
export const caseFiles: CaseFile[] = [
  ...
  },
  {
    ...
    plain: "A stress-testing tool for AI serving with authenticated benchmark agents. A deterministic CI fixture completes eight requests through two loopback agents while rejecting bad authentication and replay and accounting for sampled clock uncertainty.",
    problem: "Serving decisions need repeatable numbers: latency, throughput, failure accounting, and cost-to-serve.",
    system: "Repeatable load-generation harness for NVIDIA Triton-style and OpenAI-compatible serving with authenticated coordinator-agent execution, NTP-style clock sampling, conservative time bounds, open-loop pacing, and privacy-safe aggregate gates.",
    evidence: "CI starts two authenticated loopback agent services, two real benchmark child processes, and one synthetic SSE target. Eight requests complete with eight unique trace contexts; wrong-key and replay attempts are rejected, and conservative clock-uncertainty, start-skew, and overlap gates pass. Aggregate artifacts omit keys, URLs, child paths, endpoints, prompts, outputs, and trace IDs. This is single-host protocol evidence, not a multi-host or production-network measurement.",
...
export const proof: Proof[] = [
  ...
  { label: "authenticated benchmark agents", kind: "verified",
    plain: "A synthetic CI fixture starts two loopback agents and two benchmark clients, rejects bad authentication and replay, and gates clock uncertainty without claiming a multi-host measurement.",
