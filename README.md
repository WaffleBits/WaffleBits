...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. Its synthetic CI qualification sends streamed requests through separate local router and backend processes, injects one failure at each boundary, and reconciles retries with independent ingress/backend/success counters. The exact gate requires counters isolated to the run; aggregate agreement does not prove per-request causality.
