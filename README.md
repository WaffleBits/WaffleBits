...
- [secure-gpu-inference-gateway](https://github.com/WaffleBits/secure-gpu-inference-gateway): inference gateway demo showing the security control plane around a mock backend: access control, spending limits, audit logs, and supply-chain checks.
...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. A deterministic single-host CI fixture withholds completed responses from two agents, terminates the first coordinator, and starts a second process from an opt-in HMAC-authenticated state manifest. The second coordinator verifies and retrieves both exact completed shards while the synthetic target remains at eight unique requests. The fixture also rejects a wrong resume token and checks owner-only state plus persisted-field privacy. A separate fixture recovers one completed shard across an agent process restart with SQLite. This is synthetic protocol evidence, not partial-workflow continuation, interrupted-child recovery, multi-host behavior, or a production-network measurement.
