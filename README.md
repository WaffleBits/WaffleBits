...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. Its public CI includes a verified TLS 1.2+ path with explicit CA trust: two authenticated HTTPS agents, wrong-bearer and wrong-CA rejection before target work, and artifact privacy checks. Existing fixtures also recover one completed shard across an agent restart and both coordinator shards after response loss without duplicate target requests. This is single-host synthetic protocol evidence, not partial-workflow continuation, interrupted-child recovery, multi-host behavior, or a production-network measurement.
