...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. In its local synthetic CI fixture, a backend child exits during measured load and a separate supervisor starts a replacement on the same port. Four logical requests produce six ingress attempts, five backend receipts, four completions, two retry recoveries, and one completed restart; path, retry, trace, and lifecycle gates pass. Aggregate agreement is not per-request causality or service MTTR.
