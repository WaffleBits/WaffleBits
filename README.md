...
- Infrastructure security and reliability: authentication, authorization, RBAC, request and token-budget limits, distributed-limiter readiness evidence, workload-readiness replay, audit trails, policy enforcement, threat modeling, secure service boundaries, AI observability, sanitized trace evidence, OTLP collector export proof, Grafana dashboard provisioning, SLOs, resilience testing direction, and production extension paths such as OIDC, mTLS, external policy engines, and key management.
...
Covers authenticated model access, per-model authorization, reason-for-access enforcement, request and token-budget limiting, distributed-limiter readiness evidence for Redis/Envoy migration, deterministic workload-readiness replay, structured audit logs, Prometheus-compatible metrics, estimated input-token throughput, opt-in sanitized trace export that omits prompts, outputs, access reasons, and principal identifiers, OTLP collector-ready trace payloads, checked trace, workload-readiness, collector-payload, distributed-limiter, and capacity-plan artifacts, synthetic cost-to-serve estimates, Grafana dashboard provisioning, Kubernetes health/scrape posture, SLO notes, incident runbooks, policy checks, tests, and production extension points such as OIDC, mTLS, KMS, policy-as-code, prompt/output redaction, GPU telemetry, and external policy engines.
...
3. Extend the secure GPU inference gateway from checked trace/workload/capacity/OTLP/distributed-limiter proof to live Redis or Envoy request/token-budget controls, measured backend capacity profiles, profiling hooks, resilience probes, and captured collector/Grafana screenshots.
