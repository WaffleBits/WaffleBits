...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. Its deterministic CI fixture sends eight requests through two authenticated loopback agents and deliberately drops one successful result response. One bounded transport retry retrieves a coordinator-only in-memory result projection without executing the affected child again. Wrong-key and conflicting replay attempts are rejected; clock-uncertainty, start-skew, overlap, and trace gates pass. Aggregate artifacts omit credentials, URLs, paths, prompts, outputs, and trace IDs. This is single-host synthetic protocol evidence, not persistent recovery, multi-host behavior, or a production-network measurement.
