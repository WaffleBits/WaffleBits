...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. A deterministic CI fixture drops one successful result response, stops the agent, and starts a second process against the same opt-in SQLite state. The new process returns the completed projection with durable provenance while the synthetic target remains at two unique requests. The fixture verifies conflict rejection, database integrity, owner-only permissions, and that private inputs are not persisted. Separate CI retains the two-agent, eight-request coordination qualification. This is local synthetic protocol evidence, not coordinator restart recovery, interrupted-child recovery, multi-host storage, or a production-network measurement.
