...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. Its deterministic CI fixture starts two authenticated loopback agents and two benchmark child processes. Eight requests complete with eight unique trace contexts; wrong-key and replay attempts are rejected, and conservative clock-uncertainty, start-skew, and overlap gates pass. Aggregate artifacts omit credentials, URLs, paths, prompts, outputs, and trace IDs. This is single-host protocol evidence, not a multi-host or production-network measurement.
