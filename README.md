...
- [triton-inference-benchmark](https://github.com/WaffleBits/triton-inference-benchmark): load-generation toolkit for Triton and OpenAI-compatible serving. Its deterministic local SSE fixture launches two client processes against one planned start. Eight requests complete with eight unique trace contexts; completeness, shared-configuration, window-overlap, start-skew, and trace gates pass. Aggregate artifacts omit paths, endpoints, prompts, outputs, and trace IDs, and do not report false global latency percentiles. This is same-host process coordination, not multi-node load or cross-host clock evidence.
